<?php
	function initialize_database($link) {
		...
		mysqli_query($link, 'CREATE TABLE IF NOT EXISTS cards
		(id INT NOT NULL AUTO_INCREMENT, PRIMARY KEY(id), topic INT NOT NULL, FOREIGN KEY(topic) REFERENCES topics(id), term VARCHAR(1000) NOT NULL, answer VARCHAR(1000) NOT NULL)') or die(mysqli_error($link));
	}
	...
	function get_cards_html($link, $topic_id) {
		$r = mysqli_query($link, 'SELECT * FROM cards WHERE topic = "'.$topic_id.'"') or die(mysqli_error($link));
		if($r && mysqli_num_rows($r)) {
			while($row = mysqli_fetch_array($r)) {
			?>
